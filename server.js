...
async function start() {
  // DATABASE_URL이 설정되어 있으면(외부 DB 연결됨), 로컬 data/*.json 파일을
  // 그 안에 백업된 최신 데이터로 먼저 복원합니다. 재배포/재시작으로 로컬
  // 파일이 초기화되어도 회원가입 정보 등이 사라지지 않게 하기 위함입니다.
  await db.hydrate();
  // 프로필/작품 사진 등 업로드된 파일도 같은 이유로 복원합니다.
  await db.hydrateUploads();

  // 비어있는 데이터(학과/시설/작품/계정)만 골라서 채웁니다. 이미 채워둔 데이터(예: 직접
  // 입력한 시설 좌표, 혹은 방금 복원된 데이터)는 건드리지 않습니다.
  seed.seedMissing();
